...
//#define CHECKED_IMAGE_NAME      @"common_interest_bg_img"
//#define UNCHECKED_IMAGE_NAME    @"circum_merchant_mian_bg"
...
@interface SingleVoteCell()
- (void)changeSelection:(id)sender;
@end

@implementation SingleVoteCell
...
- (void)initWithInformation:(VoteParameter *)param withVoteTarget:(id)target andSelector:(SEL)selector
{
    ...
    for(int i = 0; i < [param.options count]; i++){
        ...
        if(i < [optionButtons count]){
            ...
            UIButton *button = buttonVote = [UIButton buttonWithType:UIButtonTypeCustom];
            button.frame = CGRectMake(OPTION_START_X, OPTION_START_Y + i * OPTION_Y_SPACE, OPTION_BUTTON_WIDTH, OPTION_BUTTON_HEIGHT);
    ...
    if(labelSummaryTitle == nil){
        ...
        buttonVote = [UIButton buttonWithType:UIButtonTypeCustom];
...
- (IBAction)changeSelection:(id)sender
{
    ...
    UIButton *oldButton = [optionButtons objectAtIndex:currentSelection];
    [oldButton setSelected:NO];
    currentSelection = [optionButtons indexOfObject:b];
    [b setSelected:YES];

    VoteOptionParameter *selectParam = [voteParameter.options objectAtIndex:currentSelection];
    
    [voteParameter.mySelection removeAllObjects];
